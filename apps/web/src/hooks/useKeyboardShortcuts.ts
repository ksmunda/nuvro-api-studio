...
export function useKeyboardShortcuts(workspaceId: string | null) {
  useEffect(() => {
    const handleKeyDown = (e: globalThis.KeyboardEvent) => {
      ...
      if (key === 's') {
        e.preventDefault();
        const saveBtn = document.getElementById('save-request-btn') as globalThis.HTMLButtonElement | null;
      ...
      if (key === 't' && !e.shiftKey) {
        e.preventDefault();
        const newTabBtn = document.getElementById('new-request-tab-btn') as globalThis.HTMLButtonElement | null;
        if (newTabBtn) {
          newTabBtn.click();
        } else if (workspaceId) {
    ...
    const handleTabNavigation = (e: globalThis.KeyboardEvent) => {
    ...
    const combinedHandler = (e: globalThis.KeyboardEvent) => {
